...
var HazelcastClient = require('hazelcast-client').Client
  ...
  , debug = require('debug')('osif-client')
;
  ...
  return new Promise(function (resolve, reject) {
    try {
      if (thisObj.clientInitialized)
        ...
        _createHazelcastClient(thisObj.databus.globalHost, thisObj.databus.globalPort)
...
function _getGlobalOpendata(key) {
  var thisClient = this;
...
function _getLocalOpendata(key) {
  var thisClient = this;
      ...
      thisClient.init()
        .then(function (thisClient) {
          if(thisClient.globalOpenDataMap) {
            var listenerList = thisClient.subscriptionList.global[key];
            if(typeof listenerList === 'undefined') {
              listenerList = [];
              thisClient.subscriptionList.global[key] = listenerList;
            }
            ...
            if( listenerList.indexOf(listener) == -1 ) {
              ...
              thisClient.globalOpenDataMap.addEntryListener(listenerObj, key, true)
        ...
        .then(function (thisClient) {
          if(thisClient.localOpenDataMap) {
            ...
            if( listenerList.indexOf(listener) == -1 ) {
              ...
              thisClient.localOpenDataMap.addEntryListener(listenerObj, key, true)
